...
export function MissionComposer({ projects, suggestedPrompt, autoFocus = false }: { projects: Project[]; suggestedPrompt?: string; autoFocus?: boolean }) {
  ...
  return <section className="rounded-2xl border bg-card p-4 shadow-sm sm:p-5" aria-labelledby="composer-title">
    <form action={action}>
      <input type="hidden" name="idempotency_key" value={idempotencyKey} />
      <div className="flex gap-3">
        <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Sparkles className="size-4" aria-hidden="true" /></span>
